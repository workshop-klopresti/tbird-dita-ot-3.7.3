...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:param name="PATH2PROJ">
    <xsl:apply-templates select="/processing-instruction('path2project-uri')[1]"
  ...
  <xsl:variable name="FILTERDOC" select="
  ...
  <xsl:template match="*[contains(@class, ' topic/shortdesc ')]" mode="outofline.abstract">
    <xsl:choose>
      <xsl:when
        test="exists(preceding-sibling::*[dita-ot:is-block(.)] | following-sibling::*[dita-ot:is-block(.)])">
        <p>
          <xsl:call-template name="commonattributes"/>
          <xsl:apply-templates/>
        </p>
  ...
  <xsl:template match="*[contains(@class, ' topic/ol ')][empty(*[contains(@class, ' topic/li ')])]"
    priority="10"/>
  <xsl:template match="*[contains(@class, ' topic/ul ')][empty(*[contains(@class, ' topic/li ')])]"
    priority="10"/>
  <xsl:template match="*[contains(@class, ' topic/sl ')][empty(*[contains(@class, ' topic/sli ')])]"
    priority="10"/>
  ...
  <!-- DL -->
  <xsl:template
    match="*[contains(@class, ' topic/dl ')][empty(*[contains(@class, ' topic/dlentry ')])]"
    priority="10"/>
  ...
  <xsl:template name="getMatchingTarget" as="element()?">
    ...
    <xsl:variable name="glossentries"
      select="$m_entry-file-contents/descendant-or-self::*[contains(@class, ' glossentry/glossentry ')]"
      as="element()*"/>
    <xsl:choose>
      ...
      <xsl:when
        test="$m_glossid = '' and $glossentries[not(@xml:lang) or normalize-space(@xml:lang) = '']">
        <xsl:sequence select="$glossentries[not(@xml:lang) or normalize-space(@xml:lang) = ''][1]"/>
      </xsl:when>
      <xsl:when
        test="not($m_glossid = '') and $glossentries[@id = $m_glossid][not(@xml:lang) or normalize-space(@xml:lang) = '']">
        <xsl:sequence
          select="$glossentries[@id = $m_glossid][not(@xml:lang) or normalize-space(@xml:lang) = ''][1]"
        />
      </xsl:when>
      <!--xsl:otherwise>
    ...
    <xsl:choose>
      <xsl:when test="@keyref and @href">
        ...
        <xsl:variable name="displaytext">
          <xsl:choose>
            <xsl:when
              test="normalize-space(.) != '' and empty(processing-instruction('ditaot')[. = 'gentext'])">
  ...
  <xsl:template match="*[contains(@class, ' topic/audio ')]">
    <audio>
      <xsl:call-template name="commonattributes"/>
      <xsl:apply-templates select="@autoplay | @controls | @loop | @muted"
        mode="boolean-media-attribute"/>
      <xsl:apply-templates select="@tabindex | @href | @format"/>
      <xsl:call-template name="setid"/>
      <xsl:apply-templates select="
          *[contains(@class, ' topic/media-source ')],
  ...
  <xsl:template match="*[contains(@class, ' topic/fn ')]" name="topic.fn">
    ...
    <xsl:if test="not(@id) or $xref = 'yes'">
      ...
      <xsl:variable name="convergedcallout" select="
  ...
  <xsl:template match="*[contains(@class, ' topic/required-cleanup ')]"
    mode="default-required-cleanup-style">
    <xsl:call-template name="style">
      <xsl:with-param name="contents">background-color: #FFFF99; color:#CC3333; border: 1pt black
        solid;</xsl:with-param>
    </xsl:call-template>
  </xsl:template>

  <xsl:template match="*[contains(@class, ' topic/draft-comment ')]"
    mode="default-draft-comment-style">
    <xsl:call-template name="style">
      <xsl:with-param name="contents">background-color: #99FF99; border: 1pt black
        solid;</xsl:with-param>
    </xsl:call-template>
  </xsl:template>

  <xsl:template match="*[contains(@class, ' topic/required-cleanup ')]"
    name="topic.required-cleanup">
  ...
  <xsl:template name="commonattributes">
    ...
    <xsl:apply-templates select="." mode="commonattributes">
      <xsl:with-param name="default-output-class"
        select="tokenize(normalize-space($default-output-class), '\s+')"/>
    </xsl:apply-templates>
  </xsl:template>
  ...
  <xsl:template match="/ | @* | node()" mode="copyright"> </xsl:template>
  ...
  <!--  Example of template calling another template; these redirects make comptibility possible for older customizations -->
  ...
  <xsl:template match="*" mode="addHeaderToHtmlBodyElement">
    <xsl:variable name="header-content" as="node()*">
      ...
      <!-- gen-user-header: include user's XSL running header here -->
      <xsl:call-template name="processHDR"/>
      <!-- processHDR: relates to build file property args.hdr -->
      <xsl:if test="$INDEXSHOW = 'yes'">
        <xsl:apply-templates select="
    ...
    <!-- if the variable header-content exists, then output a header-->
  ...
  <!--  processHDR template is a copy/paste action -->
  ...
  <xsl:template match="*" mode="ditamsg:no-glossentry-for-key">
    ...
    <xsl:call-template name="output-message">
      ...
      <xsl:with-param name="msgparams">%1=<xsl:value-of select="$matching-keys"/>;%2=<xsl:value-of
